...
@interface FMYImgDownloaderOperation ()
...
@implementation FMYImgDownloaderOperation{
    size_t width, height;
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    if ((self.options & FMYImgDownloaderProgressiveDownload) && self.expectedSize > 0 && self.completedBlock) {
        ...
        if (width + height > 0 && totalSize < self.expectedSize) {
            ...
            if (partialImageRef) {
                ...
                NSString *key = [[FMYImgManager sharedManager] cacheKeyForURL:self.request.URL];
...
//                [[NSNotificationCenter defaultCenter] postNotificationName:SDWebImageDownloadFinishNotification object:self];
    ...
    if (error) {
        ...
        FMYImgDownloaderCompletedBlock completionBlock = self.completedBlock;
        
        if (completionBlock)
        {
            if (self.options & FMYImgDownloaderIgnoreCachedResponse && responseFromCached && [[NSURLCache sharedURLCache] cachedResponseForRequest:self.request])
            {
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didReceiveChallenge:(NSURLAuthenticationChallenge *)challenge completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition disposition, NSURLCredential *credential))completionHandler {
    ...
    if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        if (!(self.options & FMYImgDownloaderAllowInvalidSSLCertificates)) {
